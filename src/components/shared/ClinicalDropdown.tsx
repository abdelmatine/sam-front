    ...
    >
      <DropdownMenu open={isOpen} onOpenChange={setIsOpen} modal={false}>
        ...
        <DropdownMenuContent 
          ...
          sideOffset={8}
          className={cn(
            "z-[160] rounded-none border-primary/20 bg-background/80 backdrop-blur-xl min-w-[220px] p-2 shadow-2xl overflow-hidden",
            className
          )}
        >
          {/* Animated Top Bar */}
                ...
                <DropdownMenuItem asChild>
                  {item.href ? (
                    <Link 
                      href={item.href}
                      className={cn(
                        "flex w-full px-4 py-3 text-[8.5px] font-bold uppercase tracking-[0.25em] transition-all border-b border-border/5 last:border-none cursor-pointer outline-none",
